                ...
                if pref.morning_time == f"{hour:02d}:00":
                    if pref.holiday_push:
                        ...
                        holiday_stmt = select(Holiday).where(
                            and_(Holiday.month == now.month, Holiday.day == now.day)
                        )
                        holidays = (await session.execute(holiday_stmt)).scalars().all()
                        for h in holidays:
                            notifications_to_create.append({
                                "user_id": user_id,
                                "title": f"🎉 {h.name} · {h.name_en}",
                                "title_en": f"🎉 {h.name_en}",
                                "body": f"今天是{h.name}！{h.description}",
                                "body_en": f"Today is {h.name_en}! {h.description_en or h.description}",
                                "type": "holiday",
                                "related_event_id": h.id,
                            })
